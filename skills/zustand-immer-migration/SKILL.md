...
`createStore` produces a vanilla store with no React dependency, so it can be
called from controllers, services, and tests without importing React.
...
1. **`createStore` vs `create`**: Use `createStore` (vanilla) for the store file;
   use `useStore` in React hooks. `create` (from `zustand`) bakes in React — the
   store becomes unusable from non-React code, which defeats the main benefit of
